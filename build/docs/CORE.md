...
- **A restore is final.** Nothing is tucked aside first and there is no button to put it back. RESONANCE moves configuration in exactly two directions — this system into the vault, the vault onto this system — and a third thing to understand is a third thing that can be misunderstood. The restore preview and the file diff are where a restore is reconsidered, and both come before it happens, not after. Note that §1 has always listed three functions and only three, and undo was never one of them. *(Removes the original "Pre-restore snapshot" line, which required the current files to be tucked aside automatically and an "Undo restore" button to bring them back. Removed v1.4.0, 2026-08-18. Amendment deployed by megas, 2026-08-18.)*
