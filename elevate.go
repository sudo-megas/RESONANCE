...
func undoSystemEntry(entry SnapshotEntry, destPath, backing string) error {
	...
	switch entry.Kind {
	case "absent":
		// The restore created this file where there had been nothing, so
		// undoing it means removing it again.
		return s.send(helperRequest{Op: "remove", Path: destPath})

	case "symlink":
		return s.send(helperRequest{Op: "symlink", Path: destPath, Link: entry.LinkTarget})

	case "regular":
		if err := refuseSymlink(backing); err != nil {
			return err
		}
	...
	default:
		return fmt.Errorf("this undo entry has a kind RESONANCE doesn't recognise (%q)", entry.Kind)
	}
}
